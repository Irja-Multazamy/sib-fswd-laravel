<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- bootstrap -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <title>Show User</title>
    <style>
        th{
            text-align : center;
        }
    </style>
</head>
<body>
    <div class="container">
        <h2 align="center">Data Pengguna</h2>
        <table>
            <tr>
                <td><a href="tambah" class="btn btn-success mt-3">Tambah Pengguna</a></td>
                <td width="50"></td>
                <td><a href="#" class="btn btn-dark mt-3">Logout</a></td>
            </tr>
        </table>
        <table class="table table-bordered" style="margin-top: 30px;">
            <thead class="thead-light">
                <tr>
                    <th>#</th>
                    <th colspan="3">Aksi</th>
                    <th>Avatar</th>
                    <th>Nama</th>
                    <th>Email</th>
                    <th>Telepon</th>
                    <th>Role</th>
                </tr>
            </thead>
                <tr>
                    <td align='center'>1</td>
                    <td><a href="#" class="btn btn-primary btn-sm btn-block">Detail</a></td> 
                    <td><a href="edit" class="btn btn-warning btn-sm btn-block">Edit</a></td>
                    <td><a href="#" onclick="return confirm('Yakin akan menghapus data ini?')" class="btn btn-danger btn-sm btn-block">Hapus</a></td>    
                    <td align='center'><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR4enEU1U3Kpwy7gyAYfoj8Te6L0iH7Dxy_1w&usqp=CAU" height="30" width="30" alt="..."></td>
                    <td align='center'>Irja Multazamy</td>
                    <td align='center'>abc@gmail.com</td>
                    <td align='center'>021060453562</td>
                    <td align='center'>Admin</td>
                </tr>
                <tr>
                    <td align='center'>2</td>
                    <td><a href="#" class="btn btn-primary btn-sm btn-block">Detail</a></td> 
                    <td><a href="#" class="btn btn-warning btn-sm btn-block">Edit</a></td>
                    <td><a href="#" onclick="return confirm('Yakin akan menghapus data ini?')" class="btn btn-danger btn-sm btn-block">Hapus</a></td>    
                    <td align='center'><img src="data:image/png;base64," height="32" width="32" alt="..."></td>
                    <td align='center'>Indah Yuliani</td>
                    <td align='center'>def@gmail.com</td>
                    <td align='center'>021042953506</td>
                    <td align='center'>Staff</td>
                </tr>
                <tr>
                    <td align='center'>3</td>
                    <td><a href="#" class="btn btn-primary btn-sm btn-block">Detail</a></td> 
                    <td><a href="#" class="btn btn-warning btn-sm btn-block">Edit</a></td>
                    <td><a href="#" onclick="return confirm('Yakin akan menghapus data ini?')" class="btn btn-danger btn-sm btn-block">Hapus</a></td>    
                    <td align='center'><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR4enEU1U3Kpwy7gyAYfoj8Te6L0iH7Dxy_1w&usqp=CAU" height="30" width="30" alt="..."></td>
                    <td align='center'>Calvin Syah</td>
                    <td align='center'>ghi@gmail.com</td>
                    <td align='center'>021072947362</td>
                    <td align='center'>Admin</td>
                </tr>
        </table>
    </div>
</body>
</html>